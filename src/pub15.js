...
export async function initPub15Page() {
  ...
  document.getElementById('select-married').addEventListener('click', async () => {
    marriedPath = await open({ multiple: false, filters: [{ name: "XML", extensions: ["xml"] }] });
    document.getElementById('married-file').textContent = marriedPath || 'No file selected';
  });

  document.getElementById('select-single').addEventListener('click', async () => {
    singlePath = await open({ multiple: false, filters: [{ name: "XML", extensions: ["xml"] }] });
    document.getElementById('single-file').textContent = singlePath || 'No file selected';
  });
